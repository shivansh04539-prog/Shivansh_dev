...
import BlogClientWrapper from "@/app/components/(Blog)/BlogClientWrapper";
...
const getSeoUrl = (path: string) => {
  const baseUrl = "https://webcontractor.in";
...
export async function generateMetadata({ params }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      title: metadata.title,
      description: metadata.summary,
      url: `https://webcontractor.in/blogs/${decodedSlug}`,
      images: [
        {
          url: seoImageUrl,
          width: 1200,
...
const SingleBlogPage = async ({ params }) => {
  ...
  const articleJsonLd = {
    ...
    "author": {
      "@type": "Person",
      "name": authorName,
      "url": metadata.author?.url || "webcontractor.inwebcontractor.in/about",
    },
    "publisher": {
      "@type": "Organization",
      "name": "Shivansh Portfolio",
      "logo": { "@type": "ImageObject", "url": "https://webcontractor.in/icon.png" }
    },
    "description": metadata.summary,
  };
